...
public struct LstsItemController {
  public func get (_ request: Request) -> EventLoopFuture<LstsItem> {
    ...
    return LstsItemModel
      ...
      .flatMapThrowing(LstsItem.init(model:))
  }
  ...
  public func update (_ request: Request) -> EventLoopFuture<LstsItem> {
    ...
    return LstsItemModel
      ...
      .unwrap(orError: Abort(.notFound))
      .flatMap{ (model) in
        model.title = itemRequest.title
        model.completedAt = itemRequest.completedAt ?? model.completedAt
        return model.update(on: request.db).transform(to: model)
      }.flatMapThrowing(LstsItem.init(model:))
  }
